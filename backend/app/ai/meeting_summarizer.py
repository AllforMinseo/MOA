...
def _build_common_rules() -> str:
    ...
    return (
        ...
        "입력 사용 기준:\n"
        '- STT transcript가 있는 경우 STT를 실제 회의 발언으로 보고, STT 내용을 중심으로 summary, decisions, action_items를 작성하세요.\n'
        '- STT transcript가 있는 경우 OCR/PDF/이미지 분석 내용은 회의 참고자료로만 사용하세요.\n'
        '- STT transcript가 있는 경우 OCR/PDF/이미지 분석에만 있고 STT에서 전혀 언급되지 않은 내용은 회의에서 논의된 것처럼 넣지 마세요.\n'
        '- 단, STT에서 문서의 번호, 제목, 항목, 키워드를 언급한 경우에는 OCR/PDF/이미지 분석 내용을 보조 근거로 연결하세요.\n'
        '- STT transcript가 없고 OCR/PDF/이미지 분석 내용만 있는 경우에는 OCR/PDF/이미지 분석 내용을 주 입력으로 보고 summary, decisions, action_items를 작성하세요.\n'
        '- STT transcript가 없을 때는 문서에 적힌 제목, 항목, 표, 체크리스트, 역할 분담, 일정, 질문 목록, 발표 자료 내용을 근거로 회의 자료의 핵심을 정리하세요.\n'
        '- STT transcript가 없을 때도 OCR/PDF/이미지 분석에 명시적으로 없는 내용은 추측하거나 추가하지 마세요.\n'
        ...
        '- STT가 없고 OCR/PDF/이미지 분석만 있는 경우에도, 문서에 명확히 적힌 담당자명만 assignee로 사용하세요.\n'
        "\n"
        ...
        '- STT가 있는 경우 OCR/PDF/이미지 분석에 있는 항목이라도 STT에서 합의나 결정으로 언급되지 않았으면 decisions에 넣지 마세요.\n'
        '- STT가 없고 OCR/PDF/이미지 분석만 있는 경우에는 문서에 "결정", "확정", "진행하기로 함", "채택", "변경", "확인 완료"처럼 명확히 적힌 항목만 decisions에 넣으세요.\n'
        '- STT가 없고 OCR/PDF/이미지 분석만 있는 경우에도 단순 참고자료 제목, 단순 설명, 질문 목록은 decisions에 넣지 마세요.\n'
        "\n"
        ...
        '- 참석자 목록이 비어 있거나 제공되지 않은 경우, STT 또는 OCR/PDF/이미지 분석에서 명확히 언급된 담당자만 assignee로 사용하세요.\n'
        ...
        '- STT가 있는 경우 OCR/PDF/이미지 분석에 있는 항목이라도 STT에서 합의나 작업 배정으로 언급되지 않았으면 action_items에 넣지 마세요.\n'
        '- STT가 없고 OCR/PDF/이미지 분석만 있는 경우에는 문서에 명확히 적힌 체크리스트, 담당자, 마감일, 준비 항목, 작업 항목, 발표 역할, 점검 항목을 action_items로 추출하세요.\n'
        '- STT가 없고 OCR/PDF/이미지 분석만 있는 경우에도 단순 목차, 단순 설명, 배경 정보는 action_items에 넣지 마세요.\n'
        "\n"

        "기한 추출 기준:\n"
        '- due_date는 STT 또는 OCR/PDF/이미지 분석에서 명확히 언급된 경우에만 작성하세요.\n'
        ...
        '- STT가 없고 OCR/PDF/이미지 분석만 있는 경우에도 표, 체크리스트, 일정란에 명확히 적힌 날짜나 마감 표현은 due_date로 사용하세요.\n'
        "\n"
